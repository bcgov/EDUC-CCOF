...
router.get(
  ...
  [param('organizationId', 'URL param: [organizationId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
...
router.put(
  ...
  [(param('organizationId', 'URL param: [organizationId] is required').notEmpty().isUUID(UUID_VERSION), checkSchema(organizationSchema))],
  (req, res) => {
    validationResult(req).throw();
