...
router.get(
  ...
  [param('enrolmentReportId', 'URL param: [enrolmentReportId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
  ...
  [
    ...
    checkSchema({
      '*.dailyEnrolmentId': {
        ...
        isUUID: { options: [UUID_VERSION], errorMessage: '[dailyEnrolmentId] must be a valid UUID' },
      },
    }),
  ],
  (req, res) => {
...
router.get(
  ...
  isValidBackendToken,
  validatePermission(PERMISSIONS.VIEW_ER),
  oneOf([query('organizationId').notEmpty().isUUID(UUID_VERSION), query('programYearId').notEmpty().isUUID(UUID_VERSION)], {
...
router.post(
  ...
  [body('enrolmentReportId', '[enrolmentReportId] is required').notEmpty().isUUID(UUID_VERSION), body('contactId', '[contactId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
