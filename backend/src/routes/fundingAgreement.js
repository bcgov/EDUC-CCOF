...
router.get(
  ...
  query('organizationId').notEmpty().withMessage('organizationId is required').isUUID(UUID_VERSION).withMessage('organizationId must be a valid UUID'),
  (req, res) => {
...
router.get(
  ...
  isValidBackendToken,
  validatePermission(PERMISSIONS.VIEW_FUNDING_AGREEMENT),
  [param('fundingAgreementId', 'URL param: [fundingAgreementId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
