...
const { getChangeRequest, createChangeRequest, createChangeRequestFacility, CHANGE_REQUEST_TYPES, deleteChangeRequest, getChangeRequestDocs, saveChangeRequestDocs, getNewFacilitiesChangeAction } = require('../components/changeRequest');
...
/**
 * Get Change Action details for a new facility
 */
router.get('/newFacility/:changeActionId', //passport.authenticate('jwt', {session: false}),isValidBackendToken,
  [param('changeActionId', 'URL param: [changeActionId] is required').not().isEmpty()], (req, res) => {
    validationResult(req).throw();
    return getNewFacilitiesChangeAction(req, res);
  });
