...
const { CHANGE_REQUEST_TYPES, PERMISSIONS, UUID_VERSION } = require('../util/constants');
...
router.get(
  ...
  validateFacility(),
  query('facilityId', 'URL query: [facilityId] is required').notEmpty().isUUID(UUID_VERSION),
  query('programYearId', 'URL query: [programYearId] is required').notEmpty().isUUID(UUID_VERSION),
  (req, res) => {
...
router.get(
  ...
  isValidBackendToken,
  validatePermission(PERMISSIONS.VIEW_CLOSURES),
  [param('changeActionClosureId', 'URL param: [changeActionClosureId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('changeRequestId', 'URL param: [changeRequestId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
...
router.post(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('changeActionId', 'URL param: [changeActionId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('ccfriId', 'URL param: [ccfriId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
...
router.delete(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('mtfiId', 'URL param: [mtfiId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
