...
const { UUID_VERSION } = require('../util/constants');
...
router.put(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('rfipfiid', 'URL param: [rfipfiid] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
...
router.put(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('nmfpfiid', 'URL param: [nmfpfiid] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
    validationResult(req).throw();
...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('applicationId', 'URL param: [applicationId] is required').notEmpty().isUUID(UUID_VERSION)],
  (req, res) => {
...
router.post(
  ...
  passport.authenticate('jwt', { session: false }),
  isValidBackendToken,
  [param('applicationId', 'URL param: [applicationId] is required').notEmpty().isUUID(UUID_VERSION), body('facilities').isArray(), body('facilities.*').isUUID(UUID_VERSION)],
  (req, res) => {
    return getApplicationSummary(req, res);
  },
);
