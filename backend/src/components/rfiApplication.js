...
async function deleteChildTable(rfipfiid, entityName, selectorName) {
  //entityName = ccof_rfi_pfi_other_fundings
  //selectorName = ccof_rfi_pfi_other_fundingid
  const query = `${entityName}?$select=${selectorName}&$filter=(_ccof_rfiparentfeeincrease_value eq ${rfipfiid})`
  try {
...
async function updateRFIApplication(req, res) {
  try {
    ...
    fundingListPayload?.forEach(async payload => {
      payload['ccof_RFIParentFeeIncrease@odata.bind'] = `/ccof_rfi_pfi_other_fundings(${rfipfiid})`;
