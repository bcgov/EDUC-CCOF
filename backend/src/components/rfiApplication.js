...
} = require("../util/mapping/MappableObject");
const {
...
async function deleteChildTable(
  rfipfiid,
  entityName,
  selectorName,
  filterName
...
async function getRFIMedian(req, res) {
  try {
    ...
    if (rfiMedian?.length > 0) {
      medians["0 to 18 months"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageof0to18;
      medians["18 to 36 months"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageof18to36;
      medians["3 Years to Kindergarten"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageof3ytok;
      medians["Out of School Care - Kindergarten"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageofoosctok;
      medians["Out of School Care - Grade 1+"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageofoosctog;
      medians["Preschool"] =
        rfiMedian[0].ccof_Region3PctMedian?.ccof_3percentageofpre;
...
async function getRFIApplication(req, res) {
  ...
  try {
    ...
    if (response.value.length > 0) {
      ...
      rfiApplication.data["expansionList"] =
        response.value[0].ccof_ccof_rfipfi_ccof_rfipfiserviceexpansiondetail_rfipfi?.map(
          (el) =>
            formatDate(
              new MappableObjectForFront(el, ServiceExpansionDetailsMappings)
                .data,
              "date"
            )
        );
      rfiApplication.data["wageList"] =
        response.value[0].ccof_rfi_pfi_dcs_wi_detail_RFI_PFI_Detail?.map((el) =>
          formatDate(
            new MappableObjectForFront(el, DCSWageIncreaseMappings).data,
            "wageDate"
          )
        );
      rfiApplication.data["expenseList"] =
        response.value[0].ccof_ccof_rfipfi_ccof_rfipfiexpenseinfo_rfipfi?.map(
          (el) =>
            formatDate(
              new MappableObjectForFront(el, ExpenseInformationMappings).data,
              "date"
            )
        );
      rfiApplication.data["fundingList"] =
        response.value[0].ccof_rfi_pfi_other_funding_RFI_PFI?.map((el) =>
          formatDate(
            new MappableObjectForFront(el, OtherFundingProgramMappings).data,
            "date"
          )
        );
      rfiApplication.data["indigenousExpenseList"] =
        response.value[0].ccof_rfipfi_ccof_rfipfi_IndegenousService?.map((el) =>
          formatDate(
...
async function updateRFIApplication(req, res) {
  try {
    const friApplication = new MappableObjectForBack(
      req.body,
      RFIApplicationMappings
    ).toJSON();
    ...
    friApplicationResponse = new MappableObjectForFront(
      friApplicationResponse,
      RFIApplicationMappings
    );
    ...
    if (req.body.fundingList) {
      await deleteChildTable(
        rfipfiid,
        "ccof_rfi_pfi_other_fundings",
        "ccof_rfi_pfi_other_fundingid"
      );
    ...
    if (req.body.wageList) {
      await deleteChildTable(
        rfipfiid,
        "ccof_rfi_pfi_dcs_wi_details",
        "ccof_rfi_pfi_dcs_wi_detailid"
      );
    ...
    if (req.body.expansionList) {
      await deleteChildTable(
        rfipfiid,
        "ccof_rfipfiserviceexpansiondetails",
        "ccof_rfipfiserviceexpansiondetailid",
        "_ccof_rfipfi_value"
      );
      const expansionListPayload = req.body.expansionList?.map(
        (el) =>
    ...
    if (req.body.expenseList) {
      await deleteChildTable(
        rfipfiid,
        "ccof_rfipfiexpenseinfos",
        "ccof_rfipfiexpenseinfoid",
        "_ccof_rfipfi_value"
      );
    ...
    if (req.body.indigenousExpenseList) {
      ...
      await deleteChildTable(
        ...
        "ccof_rfipfiserviceexpansionindigenouscommunityid",
        "_ccof_rfipfi_indegenousserviceexpansion_value"
      );
      const indigenousExpensePayload = req.body.indigenousExpenseList?.map(
        (el) => new MappableObjectForBack(el, IndigenousExpenseMappings).data
      );
      indigenousExpensePayload?.forEach(async (payload) => {
        // payload.ccof_date = formatTimeForBack(payload.ccof_date);
        payload[
          "ccof_rfipfi_IndegenousServiceExpansion@odata.bind"
        ] = `/ccof_rfipfis(${rfipfiid})`;
        await postOperation(
          "ccof_rfipfiserviceexpansionindigenouscommunities",
          payload
...
async function createRFIApplication(req, res) {
  try {
    ...
    // );
    friApplication["ccof_rfi_pfi_dcs_wi_detail_RFI_PFI_Detail"] =
      req.body.wageList?.map(
        (el) => new MappableObjectForBack(el, DCSWageIncreaseMappings).data
      );
    ...
    //     ))
    // );
    friApplication["ccof_rfi_pfi_other_funding_RFI_PFI"] =
      req.body.fundingList?.map(
        (el) => new MappableObjectForBack(el, OtherFundingProgramMappings).data
      );
    ...
    //     ))
    // );
    friApplication["ccof_ccof_rfipfi_ccof_rfipfiexpenseinfo_rfipfi"] =
      req.body.expenseList?.map(
        (el) => new MappableObjectForBack(el, ExpenseInformationMappings).data
      );
    ...
    // );
    friApplication["ccof_rfipfi_ccof_rfipfi_IndegenousService"] =
      req.body.indigenousExpenseList?.map(
        (el) => new MappableObjectForBack(el, IndigenousExpenseMappings).data
      );
    ...
    await patchOperationWithObjectId(
      "ccof_applicationccfris",
      req.params.ccfriId,
      { ccof_has_rfi: true, ccof_rfi_formcomplete: req.body.isRfiComplete }
    );
    return res
      .status(HttpStatus.CREATED)
      .json({ friApplicationGuid: friApplicationGuid });
  } catch (e) {
