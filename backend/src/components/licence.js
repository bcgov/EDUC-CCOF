...
async function getLicences(req, res) {
  try {
    let operation = 'ccof_licenses?$select=ccof_end_date,_ccof_facility_value,ccof_licenseid,ccof_name,ccof_organization,ccof_start_date';
    operation += `&$expand=ccof_service_delivery_details_license_ccof_license($select=ccof_accepts_subsidy,ccof_after_school,ccof_afternoon_kindercare,ccof_before_school,ccof_care_type,ccof_end_date,ccof_extended_hours_offered,ccof_hours_of_operation_end,ccof_hours_of_operation_start,ccof_name,
    ...
    operation += `&$filter=(_ccof_facility_value eq ${req.query.facilityId})`;
