...
async function getDeclaration(req, res) {
  try {
    const operation = `ccof_applications(${req.params.applicationId})?$select=ccof_consent,ccof_submittedby,ccof_latestsubmissiondate,ccof_declarationbstatus,statuscode,ccof_declaration_version`;
