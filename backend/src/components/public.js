...
async function getFacility(req, res) {
  try {
    ...
    let payLoad = simulatedPayloadFacility;

    /* TODO: acutal endpoint call. Uncomment and test once endpoint is implemented.
    let payLoad = (await axios.get('/api/operations?statement=accounts('+req.query.facilityId+')?'+
                                   '$select=accountid,address1_city,accountnumber,'+
                                   'name&$expand=ccof_account_ccof_parent_fees_Facility($select=ccof_parent_feesid,ccof_apr,ccof_aug,_ccof_childcarecategory_value,ccof_dec,_ccof_facility_value,ccof_feb,ccof_jan,ccof_jul,ccof_jun,ccof_mar,ccof_may,ccof_nov,ccof_oct,_ccof_programyear_value,ccof_sep),'+
                                   'ccof_facility_licenses_Facility_account($select=ccof_facility_licensesid,_ccof_facility_value,_ccof_licensecategory_value')).data;
    */

    // Iterate through the payload to colect what we need, lighten, and return a condensed payload...
    for (let x in payLoad) {
      // TODO: if statement blow is only required for hardcoded payload. Remove when no longer needed remove from the for loop it surrounds.
      if (payLoad[x].accountid == req.params.facilityId) {
        // Get the header record info from the payload. NOTE: we are dong this because we are transforming the payload to into something lighter
        // to return than original payload...
        results.facilityId = payLoad[x].accountid;
        results.name = payLoad[x].name;
        results.city = payLoad[x].address1_composite;
        results.approvedFeesByChildAgeCategory = [];
        approvedFeesByChildAgeCategory = payLoad[x].ccof_account_ccof_parent_fees_Facility;
 
        // For each approvedFeesByChildAgeCategory payload rec, transform the payload data into less verbose json...
        let rec;
