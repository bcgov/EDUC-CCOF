...
export function isValidEndDate({ startDate, endDate }, delimiter = '/') {
  ...
  if (startDate?.length && endDate?.length && startDate?.length === endDate?.length) {
    let parseJoda;
    if (endDate.length === 8) {
      // if the length is 8 then there was no delimiter used.
...
export function isValidLength(length, required = true) {
  ...
  return [...rules, (str) => !str || (str && str.length <= length) || 'Max ' + length + ' characters'];
}
...
export function isValidEmail(value) {
  return !!(
    value && /^[\w!#$%&’*+/=?`{|}~^-]+(?:\.[\w!#$%&’*+/=?`{|}~^-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,6}$/.test(value)
  );
}
