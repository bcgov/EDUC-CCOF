<template>
  <v-sheet class="flex px-0 py-0" elevation="0" max-width="337" min-width="150" style="float:left">
    ...
    <!-- ****************************************************************************************************************************************************************-->
    <v-slide-group v-if="isActive" v-model="selectedMonthIndex" class="pa-0" mandatory center-active show-arrows active-class="" @change="focusAwayFromOnSlider">
      ...
      <template v-slot:prev>
        <span class="estimator-label fill-height pt-1 pr-0" align="right">
          <v-icon color="#39598A" x-large>mdi-chevron-left</v-icon>
            Daily&nbsp;
          </span>
      </template>
      <v-slide-item v-for="n in items" :key="n.id" v-slot="{ active, toggle }">
        <v-card :color="active ? '#E5F3FE' : '#FFFFFF'" class="ma-1 fill-height" :elevation="active ? 4 : 0" height="67" width="70" @click="toggle(clickForOnSlider(n.id))">
          <v-row style="" justify="center">
            <v-col align="center" style="padding-top:4px;padding-bottom:5px;margin-top:-2px;">
              <v-div :style="'color:'+(active ? '#2196f3' : '#39598A')+';font-family:Lucida Grande,monospace;color:#39598A;background-color:#EEEEEE;font-size:17px;font-weight:bold;padding-bottom:6px;padding-left:18px;padding-right:18px;padding-top:4px'">
               {{n.month}}
              </v-div>
            </v-col>
          </v-row>
    ...
    <!-- ****************************************************************************************************************************************************************-->
    <v-slide-group v-if="!isActive" v-model="selectedMonthIndex" class="pa-0" mandatory center-active show-arrows active-class="" @change="">
      <template v-slot:next>
        <span class="fill-height pt-1 pr-4">
      ...
      <template v-slot:prev>
            <span class="estimator-label fill-height pt-1 pr-0" align="right" style="font-size:12px;">
              <v-icon color="#39598A" x-large>mdi-chevron-left</v-icon>
                Monthly&nbsp;
              </span>
      </template>
      <v-slide-item v-for="n in items" :key="n.id" v-slot="{ active, toggle }">
      <v-card :color="active ? '#FFFFFF': '#FFFFFF'" class="ma-1 fill-height" :elevation="active ? 0 : 0" height="67" width="70" @click="toggle(clickForOffSlider(n.id))">
        <v-row style="" justify="center">
          <v-col align="center" style="padding-top:4px;padding-bottom:5px;margin-top:-2px;">
