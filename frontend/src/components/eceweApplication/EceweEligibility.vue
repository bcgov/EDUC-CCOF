    ...
    <v-container>
      ...
      <v-row class="mt-6 justify-center">
        <v-alert
          ...
          <span>
            <strong>Note:</strong> Please read and understand the full eligibility requirements in the <u>ECE-WE <a :href="fundingUrl"  target="_blank">Funding Guidelines</a></u>.  All CCFRI-eligible facilities must opt-in to CCFRI <u>to be eligible for ECE-WE.</u>
          </span>
        </v-alert>
      </v-row>
      ...
      <v-row v-if="(model.belongsToUnion == 1 && model.optInECEWE == 1 && model.publicSector == 1 && languageYearLabel == programYearTypes.FY2024_25) || (model.belongsToUnion == 1 && model.optInECEWE == 1 && languageYearLabel == programYearTypes.HISTORICAL) || isLoading" class="justify-center">
        <v-card elevation="4" class="py-2 px-5 mx-2 mt-10 rounded-lg col-11">
          <v-container>
            <v-row v-if="isLoading">
              <v-col>
