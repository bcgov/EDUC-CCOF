...
*  
* Source: Copied from commit 1146a77 on Dec 19, 2024 with some minor changes.
-->
<template>
      ...
      <v-skeleton-loader :loading="loading" type="table-tbody" class="mb-12">
        <v-row justify="space-around">
          ...
          <v-card v-if="model.maxGroupChildCareSchool > 0" class="cc-top-level-card" width="1200">
            ...
            <v-card-title v-show="model.isSchoolProperty">
              Group Child Care (School Age Care on School Grounds)
            </v-card-title>

            <v-card-subtitle v-show="model.isSchoolProperty">
              Please indicate each service that your facility offers
            </v-card-subtitle>

            <v-container v-show="model.isSchoolProperty">
              <v-row>
                <v-col>
