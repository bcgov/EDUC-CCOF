<template>
  ...
  <v-form v-else ref="form" v-model="isValidForm" class="mx-8 mb-12 mx-lg-16">
    ...
    <div class="my-8">
      <p>
        The Funding Agreement outlines the legal terms and conditions to participate in and receive Child Care Operating
        Funding (CCOF) Base Payment and if applicable, Child Care Fee Reduction Initiative (CCFRI) and Early Childhood
        Educator Wage Enhancement (ECE-WE).
      </p>
      <p class="my-4">
        Review the Funding Agreement below in full to understand the requirements, payments, and obligations of both you
        and the Province before proceeding with your CCOF, CCFRI and/or ECE-WE renewal. Each licence eligible to receive
        funding under this organization is shown as a separate Schedule A in the Funding Agreement.
      </p>
      <p>Also, check that:</p>
      <ul class="ml-5">
      ...
      <p class="my-4">
        Notify the Child Care Operating Funding Program of a change to organization information, licences or Schedule A
        information.
      </p>
    </div>
    ...
    <v-card v-if="fundingAgreement" class="px-8 px-lg-12 py-4 mb-8">
      ...
      <v-card class="card-top-border-primary my-8 px-8 py-6">
        ...
        <v-card v-if="isFundingAgreementConfirmed === YES_NO_VALUES.NO" rounded="0" class="mb-4">
          ...
          <p class="px-8 py-4">
            Submit a
            <router-link class="text-decoration-underline" :to="PATHS.ROOT.CHANGE_LANDING">change request</router-link>.
    ...
    <v-card v-if="isFundingAgreementConfirmed">
      ...
      <div class="px-8 px-lg-12 pt-6">
        <p>
          ...
          Operating Funding Program of a change to your Facility Licence or Child Care Service Details by submitting a
          change request.
        </p>
        <div class="d-flex justify-end my-4">
          <AppButton :loading="isApplicationProcessing" size="medium" @click="goToChangeRequest">
