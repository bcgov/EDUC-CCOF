      ...
      <v-row justify="space-around">
        <v-card class="cc-top-level-card" width="1200">
          <v-container>
            ...
            <v-row v-show="hasClosedMonth === 'yes'">
              <v-col>
                <v-checkbox v-model="closedMonths" value="1" label="Jan" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="2" label="Feb" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="3" label="Mar" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="4" label="Apr" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="5" label="May" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="6" label="Jun" />
              </v-col>
            </v-row>

            <v-row v-show="hasClosedMonth === 'yes'">
              <v-col>
                <v-checkbox v-model="closedMonths" value="7" label="Jul" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="8" label="Aug" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="9" label="Sep" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="10" label="Oct" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="11" label="Nov" />
              </v-col>
              <v-col>
                <v-checkbox v-model="closedMonths" value="12" label="Dec" />
              </v-col>
            </v-row>

            <v-row>
              <v-col cols="12" md="6">
                <v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" :nudge-right="40" :return-value.sync="hoursFrom" transition="scale-transition" offset-y max-width="290px" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field outlined required :rules="rules.required" v-model="hoursFrom12hr" label="Facility hours of operation From" readonly v-bind="attrs" v-on="on">
                    </v-text-field>
                  </template>
                  <v-time-picker v-if="menu1" v-model="hoursFrom" full-width @click:minute="$refs.menu1.save(hoursFrom); hoursFrom12hr = formatTime(hoursFrom)" :allowed-minutes="allowedStep" />
                </v-menu>
              </v-col>
              <v-col cols="12" md="6">
                <v-menu ref="menu2" v-model="menu2" :close-on-content-click="false" :nudge-right="40" :return-value.sync="hoursTo" transition="scale-transition" offset-y max-width="290px" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field outlined required :rules="rules.required" v-model="hoursTo12hr" label="Facility hours of operation To" readonly v-bind="attrs" v-on="on">
                    </v-text-field>
                  </template>
                  <v-time-picker v-if="menu2" v-model="hoursTo" full-width @click:minute="$refs.menu2.save(hoursTo); hoursTo12hr = formatTime(hoursTo)" :allowed-minutes="allowedStep" />
                </v-menu>
              </v-col>
            </v-row>
          </v-container>
        ...
        <v-card class="cc-top-level-card" width="1200">
          ...
          <v-container>
            ...
            <v-row>
              <v-col>
                <v-text-field type="number" outlined required :rules="rules.required" v-model.number="monday" label="Monday" />
              </v-col>
              <v-col>
                <v-text-field type="number" outlined required :rules="rules.required" v-model.number="tusday" label="Tuesday" />
              </v-col>
              <v-col>
                <v-text-field type="number" outlined required :rules="rules.required" v-model.number="wednesday" label="Wednesday" />
              </v-col>
              <v-col>
                <v-text-field type="number" outlined required :rules="rules.required" v-model.number="thursday" label="Thursday" />
              </v-col>
              <v-col>
                <v-text-field type="number" outlined required :rules="rules.required" v-model.number="friday" label="Friday" />
              </v-col>
              <v-col>
        ...
        <v-card class="cc-top-level-card" width="1200">
          <v-container>
            ...
            <v-row v-show="isExtendedHours === 'yes'">
              <v-col>
                <v-text-field type="number" outlined required :rules="isExtendedHours === 'yes' ? rules.required : []" v-model.number="maxDaysPerWeekExtended" label="Maximum number of days per week you offer extended hours of child care?" />
              </v-col>
            </v-row>

            <v-row v-show="isExtendedHours === 'yes'">
              <v-col>
                <v-text-field type="number" outlined required :rules="isExtendedHours === 'yes' ? rules.required : []" v-model.number="maxDaysPerYearExtended" label="Maximum number of weeks per year you offer extended hours of child care?" />
              </v-col>
            </v-row>
