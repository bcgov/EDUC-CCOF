        ...
        <template #content>
          ...
          <div v-if="!isCCOFApproved">
            <v-container class="px-0" v-for="item in ccofNewApplicationText" :key="item.infoTitle" fluid>
              ...
              <v-card color="#B3E5FF" class="mt-1 pa-1 py-2" outlined v-if="ccofStatus === CCOF_STATUS_NEW"
          ...
          <p class="pt-2" v-if="ccofStatus === CCOF_STATUS_NEW">
            ...
            <a class='text-decoration-underline'
              href="https://www2.gov.bc.ca/gov/content/family-social-supports/caring-for-young-children/running-daycare-preschool/child-care-operating-funding">gov.bc.ca/childcareoperatingfunding</a>
          </p>
        </template>
        <template #button>
          <v-btn dark class="blueButton" @click="newApplicationIntermediatePage()" v-if="ccofStatus === CCOF_STATUS_NEW">Start
            Application</v-btn>
          <v-btn dark class="blueButton" @click="actionRequiredOrganizationRoute()"
            v-else-if="ccofStatus === CCOF_STATUS_ACTION_REQUIRED">Update your PCF</v-btn>
          ...
          <div v-else>
            <p class="text-h5 blueText mb-0" v-if="ccofStatus === CCOF_STATUS_APPROVED">Status of your funding agreement
              for the current fiscal year: Approved</p>
            <p class="text-h5 blueText mb-0" v-else>Status: Submitted</p>
            <v-btn dark class="blueButton mt-4" @click="viewApplication('NEW')" v-if="applicationType === 'NEW'">View
              Application</v-btn>
          </div>
        </template>
      </SmallCard>

      <SmallCard :class="smallCardLayout('RENEW')" :title="`Renew my funding agreement for ${this.renewalYearLabel}`"
        :disable="!isRenewEnabled">
        <template #content>
          ...
          <p><b>If you want to renew and can't select 'Renew my Funding Agreement', you may be using the incorrect
              BCeID.</b></p>
          <p><b>Please call 1-888-338-6622 (Option 2) if you are unsure wich BCeID to use.</b></p>
          ...
          <div v-else-if="ccofRenewStatus === RENEW_STATUS_COMPLETE">
            ...
            <span>We will contact you if we require further information. You can view your latest submission from the
              button below.</span>
          </div>
        </template>
        <template #button>
          <v-btn :color='buttonColor(!isRenewEnabled)' dark v-if="ccofRenewStatus === RENEW_STATUS_NEW"
            @click="renewApplication()">Renew my Funding Agreement</v-btn>
          <v-btn :color='buttonColor(!isRenewEnabled)' dark v-else-if="ccofRenewStatus === RENEW_STATUS_CONTINUE"
            @click="continueRenewal()">Continue Renewal</v-btn>
          <v-btn :color='buttonColor(!isRenewEnabled)' dark v-else-if="ccofRenewStatus === RENEW_STATUS_ACTION_REQUIRED"
            @click="actionRequiredOrganizationRoute()">Update your PCF</v-btn>
      ...
      <SmallCard :class="smallCardLayout('OTHERS')" :disable="!isCCOFApproved">
        <template #content>
          ...
          <p>
            ...
            contact the Child Care Operating Funding program before submitting your next enrolment report or monthly ECE
            report.
          </p>
        </template>
        <template #button>
          <v-btn href="https://childcareinfo.gov.bc.ca/childcare/welcome_ccof.aspx" :color='buttonColor(!isCCOFApproved)'
            dark>Submit a report</v-btn>
        </template>
      </SmallCard>
    </v-row>
      ...
      <v-row v-if="navBarList?.length > 2" no-gutters>
        <v-col class="col-12 col-md-6 px-4 mt-4">
          <!--TODO: sezarch box only looks at facility name. Update it later to search for status and licence
            ...
            .-->
          <v-text-field clearable filled label="Filter by Facility Name " v-model="input" :bind="input">
          </v-text-field>
        </v-col>
      </v-row>
      <v-row no-gutters justify="space-around">
        <v-col class="col-12 col-xl-6 pa-4 flex d-flex flex-column"
          v-for="({ facilityName, facilityId, ccfriApplicationId, ccfriStatus, eceweStatus, ccfriOptInStatus, eceweOptInStatus, facilityAccountNumber, licenseNumber }) in filteredList"
          :key="facilityId">
          <v-card class="elevation-4 pa-2 rounded-lg blueBorder flex d-flex flex-column" min-height="230">
            <v-card-text>
              <p class="text-h5 text--primary text-center" v-if="facilityAccountNumber">Facility ID:
                {{ facilityAccountNumber }}</p>
            ...
            <v-row justify="center" no-gutters class="mb-4"
              v-if="isCCFRIUnlock(ccfriApplicationId) || isNMFUnlock(ccfriApplicationId) || isRFIUnlock(ccfriApplicationId)">
              <v-btn class="blueButton" dark width="80%" align="center"
                @click="actionRequiredFacilityRoute(ccfriApplicationId)">Update your PCF</v-btn>
            </v-row>
          </v-card>
        </v-col>
      </v-row>
