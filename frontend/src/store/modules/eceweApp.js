...
export default {
  ...
  actions: {
    ...
    async saveECEWE({ state, commit }) {
      checkSession();
      try {
        console.info('ECEWEApp.state.eceweModel = ');
        console.info(JSON.parse(JSON.stringify(state.eceweModel)));
        console.info('ECEWEApp.state.loadedModel = ');
    ...
    initECEWEFacilities({ state, commit }, navBarList) {
      let facilityPayload;
      if (state.eceweModel.facilities?.length == 0) {
        // Create the ECEWE facility payload from the narBarList.
        facilityPayload = new Array(navBarList.length).fill({});
        for (let i = 0; i < navBarList.length; i++) {
          facilityPayload[i] = {eceweApplicationId: null, facilityId: navBarList[i].facilityId, optInOrOut: (state.eceweModel.fundingModel == state.fundingModelTypes[0].id)?0:null};
        }
      } else {
        ...
        for (let j = 0; j < navBarList.length; j++) {
          facilityPayload[j] = {facilityId: navBarList[j].facilityId,
      ...
      function getEceweApplicationId(facilityId) {
        const index = state.eceweModel.facilities.map(facilty => facilty.facilityId).indexOf(facilityId);
        return (index >= 0)?state.eceweModel.facilities[index].eceweApplicationId:null;
      }
      function getOptInOrOut(facilityId) {
        if (state.eceweModel.fundingModel == state.fundingModelTypes[0].id) {
          ...
          const index = state.eceweModel.facilities.map(facilty => facilty.facilityId).indexOf(facilityId);
          return (index >= 0)?state.eceweModel.facilities[index].optInOrOut:null;
        }
      }
    }
  },
